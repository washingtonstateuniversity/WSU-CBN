<?php
/*
Plugin Name: Connections Tile Plus - Template
Plugin URI: http://www.connections-pro.com
Description: Connections Tile Plus - Template
Version: 3.0.1
...
if ( ! class_exists( 'CN_TilePlus' ) ) {
	...
	class CN_TilePlus {
		...
		public function __construct() {
			...
			$atts = array(
				...
				'description' => 'Business card style with fixed width and height which will float to create columns that include category select and pagination.',
	...
	class CNT_TilePlus {
		...
		public static function enqueueCSS() {
			...
			wp_enqueue_style( 'cnt_' . self::$template->getSlug() . '-css', self::$template->getURL() . "tile-plus$min.css", array( 'connections-user' ), self::$template->getVersion() );
		}
		...
		public static function enqueueJS() {
			...
			wp_enqueue_script( 'cnt_' . self::$template->getSlug() . '-js' , self::$template->getURL() . "tile-plus$min.js", array( 'jquery-chosen-min' ), self::$template->getVersion(), TRUE );
		}
		...
		public static function initTemplateOptions( $atts ) {
			...
			$convert->toBoolean( $atts['show_image'] );
		...
		if ( class_exists('connectionsLoad') ) {
			...
			add_action(
				'admin_notices',
				 create_function(
				 	'',
					'echo \'<div id="message" class="error"><p><strong>ERROR:</strong> Connections must be installed and active in order use Connections Tile Plus.</p></div>\';'
					)
			);
	 ...
	 */
	add_action( 'plugins_loaded', 'Connections_TilePlus', 11 );
}