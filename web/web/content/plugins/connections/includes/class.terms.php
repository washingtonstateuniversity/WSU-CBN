...
class cnTerms
{
	...
	public function addTerm($term, $taxonomy, $attributes)
	{
		...
		$sql = "INSERT INTO " . CN_TERMS_TABLE . " SET
			name    	= '" . $wpdb->prepare($term) . "',
			slug    	= '" . $wpdb->prepare($slug) . "',
			term_group	= '0'";
		...
		$sql = "INSERT INTO " . CN_TERM_TAXONOMY_TABLE . " SET
			...
			taxonomy   	= '" . $wpdb->prepare($taxonomy) . "',
			description	= '" . $wpdb->prepare($description) . "',
			count		= '0',
			parent		= '" . $wpdb->prepare($parent) . "'";
	...
	public function updateTerm( $termID, $taxonomy, $attributes ) {
		...
		$sql = "UPDATE " . CN_TERMS_TABLE . " SET
				slug		= ''
				WHERE term_id = '" . $wpdb->prepare( $termID ) . "'";
		...
		$sql = "UPDATE " . CN_TERM_TAXONOMY_TABLE . " SET
			term_id		= '" . $wpdb->prepare( $termID ) . "',
			taxonomy	= '" . $wpdb->prepare( $taxonomy ) . "',
			description	= '" . $wpdb->prepare( $description ) . "',
			parent		= '" . $wpdb->prepare( $parent ) . "'
			WHERE term_taxonomy_id 	= '" . $wpdb->prepare( $ttID ) . "'";
