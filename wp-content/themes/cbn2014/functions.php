...
function set_js_theme_path() {
	wp_enqueue_script( 'jquery' );
	echo "<script type='text/javascript'>var THEME_PATH='".get_template_directory_uri()."';</script>";
...
function load_theme_js() {
	if (!is_admin()) {
		...
		wp_style_add_data( 'html5', 'conditional', 'lt IE 9' );
...
/*
		...
		wp_enqueue_script( 'iniscript',  $THEME. '/js/ini.js', array( 'jquery','cn-form-ui-user','gumby.init' ), '2014.25.4', true );
...
function load_theme_css() {
	if (!is_admin()) {
		...
		wp_enqueue_style( 'style.main', $THEME . '/css/style.css', array(), '2014.12.3' );
	}
}
